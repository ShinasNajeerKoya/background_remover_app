// custom colors of the app

import 'package:flutter/material.dart';

class AppColors {
  AppColors._();

  static const Color kTransparent = Color(0x00000000);
  static const Color kBlack = Color(0xFF000000);
  static const Color kCarousalPrimaryCream = Color(0xFFC67B56);
  static const Color kCarousalSecondaryCream = Color(0xFFEFCCC4);
  static const Color kCreamBackgroundColor = Color(0xFFFBF9F3);
  static const Color kDisableColor = Color(0xFFD1D1D1);
  static const Color kHomeBlackColor = Color(0xFF1F1F1F);
  static const Color kHomeBgColor = Color(0xFFFFFDFB);
  static const Color kHomeGraphBgColor = Color(0xFFF5F4ED);

  // prestige pixels

  static const Color kSplashBlackBgLeftColor = Color(0xFF191919);
  static const Color kSplashBlackBgRightColor = Color(0xFF000000);
  static const Color kPrimaryGoldColor = Color(0xFFAD925F);
  static const Color kHomeLogoGoldColor = Color(0xFFF3D496);
  static const Color kHomeBorderGoldColor = Color(0xFFCCAC6C);
}

final List<Color> lightColors = [
  Color(0xFFFCF6D7), // light yellow
  Color(0xFFFFEBC7), // light orange
  Color(0xFFC9F8FF), // light cyan
  Color(0xFFDAFFDB), // light green
  Color(0xFFFFD7DC), // light red/pink
  Color(0xFFE9DAFF), // light purple
];

final List<Color> homeGoldenGradient = [
  Color(0xFFF3D496), // light yellow
  Color(0xFFC9A968), // light purple
];
